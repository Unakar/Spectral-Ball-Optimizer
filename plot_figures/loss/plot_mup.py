...
def plot_single_optimizer(ax, df: pd.DataFrame, title: str, show_legend: bool = True):
    """绘制单个优化器的 muP 图"""
...
def plot_single_optimizer_broken_axis(axes_list, df: pd.DataFrame, title: str):
    """使用断轴绘制单个优化器的 muP 图，每个 width 一个子图"""
    
    hidden_sizes = sorted(df['hidden_size'].unique())
    n_widths = len(hidden_sizes)
    ...
    for i, hidden_size in enumerate(hidden_sizes):
        ...
        # 自动设置 y 轴范围，让曲线占满空间
        y_margin = (losses.max() - losses.min()) * 0.3
        ax.set_ylim(losses.min() - y_margin, losses.max() + y_margin)
        
        # 在右侧显示 width 标签
        ax.text(1.02, 0.5, f'w={hidden_size}', transform=ax.transAxes,
    ...
    # 设置标题（在最顶部的子图）
    axes_list[0].set_title(title, fontweight='bold', pad=10)
    
    # 设置共享的 y 轴标签
    fig = axes_list[0].figure
    fig.text(0.02, 0.5, 'Val Loss', va='center', rotation='vertical', 
             fontsize=12, fontweight='bold')


def plot_mup_comparison_broken():
    """使用断轴绘制 muon 和 spball 的 muP 对比图"""
    
    # 加载数据
    df_muon = load_and_process_data(MUON_CSV) if MUON_CSV.exists() else None
    df_spball = load_and_process_data(SPBALL_CSV) if SPBALL_CSV.exists() else None
    
    n_widths = 4  # 假设有 4 个 width
    
    # 创建图形：2 列，每列 4 行（每个 width 一行）
    fig, axes = plt.subplots(n_widths, 2, figsize=(14, 10), sharex='col')
    
    # 绘制左侧 Muon
    if df_muon is not None:
        plot_single_optimizer_broken_axis(axes[:, 0], df_muon, 'Muon Optimizer')
    
    # 绘制右侧 Spectral Sphere
    if df_spball is not None:
        plot_single_optimizer_broken_axis(axes[:, 1], df_spball, 'Spectral Sphere Optimizer')
    
    # 添加断轴标记
    d = 0.015  # 断轴标记大小
    for col in range(2):
        for row in range(n_widths - 1):
            ax = axes[row, col]
            ax2 = axes[row + 1, col]
    ...
    # 添加统一图例
    handles = [
        Line2D([0], [0], color=COLORS[256], marker='o', markersize=6, linewidth=2, label='width=256'),
        Line2D([0], [0], color=COLORS[512], marker='o', markersize=6, linewidth=2, label='width=512'),
        Line2D([0], [0], color=COLORS[1024], marker='o', markersize=6, linewidth=2, label='width=1024'),
        Line2D([0], [0], color=COLORS[2048], marker='o', markersize=6, linewidth=2, label='width=2048'),
        Line2D([0], [0], marker='*', color='gray', markersize=15, linestyle='None',
               markeredgecolor='white', markeredgewidth=1, label='Min Loss'),
    ]
    ...
    plt.subplots_adjust(left=0.08, right=0.95, top=0.95, bottom=0.12, hspace=0.08)
    
    # 保存
    output_file = OUTPUT_DIR / "mup_comparison_broken.pdf"
...
def plot_mup_comparison():
    ...
    if MUON_CSV.exists():
        ...
        axes[0].text(0.5, 0.5, 'muon_results.csv not found', 
                     ha='center', va='center', transform=axes[0].transAxes)
    
    if SPBALL_CSV.exists():
        df_spball = load_and_process_data(SPBALL_CSV)
        plot_single_optimizer(axes[1], df_spball, 'Spectral Sphere Optimizer', show_legend=False)
    else:
        axes[1].text(0.5, 0.5, 'spball_results.csv not found', 
...
def main():
    ...
    # # 绘制断轴版本对比图（每个 width 独立 y 轴范围，下凹更明显）
    # print("\n生成断轴版本对比图...")
    # plot_mup_comparison_broken()
